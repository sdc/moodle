 ...
 */
function xmldb_bcgtcg_uninstall()
{
    ...
    $quals = $DB->get_records_sql("SELECT qual.id FROM {block_bcgt_qualification} AS qual
        ...
        WHERE type.bcgttypefamilyid = ?", array(4));
    if($quals)
    {
    ...
    $types = $DB->get_records_sql("SELECT id FROM {block_bcgt_type} 
        WHERE bcgttypefamilyid = ?", array(4));
    if($types)
    {
    ...
    $DB->delete_records('block_bcgt_type_family', array("id"=>4));
    
    for($i = 1; $i <= 3; $i++){
        $DB->delete_records('block_bcgt_pathway_dep', array("id" => $i));
    }
    
    for($i = 1; $i <= 5; $i++){
        $DB->delete_records('block_bcgt_pathway_type', array("id" => $i));
    }
    
    for($i = 1; $i <= 4; $i++){
        $DB->delete_records('block_bcgt_pathway_dep_type', array("id" => $i));
    }
    
    $DB->delete_records('block_bcgt_subtype', array("id"=>14));
    $DB->delete_records('block_bcgt_subtype', array("id"=>15));
    
    for($i = 1; $i <= 8; $i++){
        $DB->delete_records('block_bcgt_pathway_subtype', array("id" => $i));
    }
