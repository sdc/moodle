 ...
 * 
 */
class CGHBNVQQualification extends CGQualification {
 
    const ID = 11;
    const FAMILYID = 4;
    const NAME = 'CG HB NVQ';
    ...
    public function insert_qualification()
	{
		...
		$targetQualID = parent::get_target_qual(CGHBNVQQualification::ID);
...
JS;
    ...
    public function get_criteria_met_value()
    {
        ...
        $params = array(CGHBNVQQualification::ID, 'A');
    ...
    public function get_max_sub_criteria_of_criteria($name)
    {
        ...
        foreach($this->units as $unit)
        {
            ...
            $criteria = $unit->findCriteriaByNameFromArray($unit->get_criteria(), $name);
            ...
            {
                if(!in_array($subCriteria->get_name(), $array) && $subCriteria->get_number_of_observations() > 0)
                {
                    $array[] = $subCriteria->get_name();
                } 
            }            
    ...
    private function get_sub_criteria_header($criteriaNames)
    {
        ...
        $output .= "<tr>";
        $output .= "<th></th><th></th><th></th>"; #unitcomments, unit name, award cols
        ...
        foreach($criteriaNames as $criteriaName)
        {
            ...
            if($subCriteria)
            {
                ...
                // Only outcomes in the subcriteria header, not standard sub criteria
    ...
    protected function get_grid_header($totalCredits, $studentView, $criteriaNames, $grid, $subCriteriaArray = false, $printGrid = false)
	{
        ...
        $header .= "<th>Sign-off</th>"; # Signoff sheet col
        $header .= "<th>IV</th>"; # IV col
            ...
            $studentView)
    {
        ...
        foreach($units AS $unit)
        {
			...
			{	
                ...
                $unit->load_student_information($this->studentID, $this->id, $loadParams);
                ...
                if($criteriaNames)
				{
					...
					{	
						...
						if($studentView)
						{
							...
							if($studentCriteria)
							{	
								$retval .= $studentCriteria->get_grid_td_($editing, $advancedMode, $unit, $user, $this, 'student', $listOfOutcomes, $colspan);
...
//                            $rowArray[] = $retval;
    ...
    public static function get_grid_key($string = true)
	{
        ...
        $possibleValues = CGQualification::get_possible_values(CGHBNVQQualification::ID, true);
    ...
    protected function get_required_settings()
    {
        ...
        $output .= "<table style='width:60%;margin:auto;'>";
            $output .= "<tr><td>C&G Registration Number:</td><td><input type='text' name='CGRegNo' onblur='updateUserSetting(this, {$this->studentID}, {$this->id});return false;' value='".$this->get_attribute('CGRegNo', $this->studentID)."' /></td><td><span id='cgregnoid' class='saveSetting'>Saved</span></td></tr>";
            $output .= "<tr><td>C&G Registration Date <small>(dd-mm-yyyy)</small>:</td><td><input type='text' name='CGRegDate' onblur='updateUserSetting(this, {$this->studentID}, {$this->id});return false;' value='".$this->get_attribute('CGRegDate', $this->studentID)."' /></td><td><span id='cgregdateid' class='saveSetting'>Saved</span></td></tr>";
            if ( has_capability('block/bcgt:editstudentgrid', $context)) $output .= "<tr><td>Expected End Date <small>(dd-mm-yyyy)</small>:</td><td><input type='text' name='ExpectedEnd' onblur='updateUserSetting(this, {$this->studentID}, {$this->id});return false;' value='".$this->get_attribute('ExpectedEnd', $this->studentID)."' /></td><td><span id='expectedend' class='saveSetting'>Saved</span></td></tr>";
            if ( has_capability('block/bcgt:editstudentgrid', $context)) $output .= "<tr><td>Assessor(s):</td><td><input type='text' name='Assessors' onblur='updateUserSetting(this, {$this->studentID}, {$this->id});return false;' value='".$this->get_attribute('Assessors', $this->studentID)."' /></td><td><span id='assessors' class='saveSetting'>Saved</span></td></tr>";
            if ( has_capability('block/bcgt:editstudentgrid', $context)) $output .= "<tr><td>Internal Verifier(s):</td><td><input type='text' name='IVs' onblur='updateUserSetting(this, {$this->studentID}, {$this->id});return false;' value='".$this->get_attribute('IVs', $this->studentID)."' /></td><td><span id='ivs' class='saveSetting'>Saved</span></td></tr>";
            if ( has_capability('block/bcgt:editstudentgrid', $context)) $output .= "<tr><td>Cross Unit Knowledge <small>(dd-mm-yyyy)</small>:</td><td><input type='text' name='CrossUnitKnowledge' onblur='updateUserSetting(this, {$this->studentID}, {$this->id});return false;' value='".$this->get_attribute('CrossUnitKnowledge', $this->studentID)."' /></td><td><span id='crossunitknowledge' class='saveSetting'>Saved</span></td></tr>";
        $output .= "</table>";
        ...
        if (has_capability('block/bcgt:editstudentgrid', $context))
        {
            ...
            $output .= "<table style='width:60%;margin:auto;'>";
                $output .= "<tr><td>Centre Name:</td><td><input type='text' name='CGCentreName' onblur='updateQualAttribute(this, {$this->id});return false;' value='".$this->get_attribute('CGCentreName')."' /></td><td><span id='cgcentrename' class='saveSetting'>Saved</span></td></tr>";
                $output .= "<tr><td>Centre Number:</td><td><input type='text' name='CGCentreNum' onblur='updateQualAttribute(this, {$this->id});return false;' value='".$this->get_attribute('CGCentreNum')."' /></td><td><span id='cgcentrenum' class='saveSetting'>Saved</span></td></tr>";
                $output .= "<tr><td>Centre Address:</td><td><input type='text' name='CGCentreAddr' onblur='updateQualAttribute(this, {$this->id});return false;' value='".$this->get_attribute('CGCentreAddr')."' /></td><td><span id='cgcentreaddr' class='saveSetting'>Saved</span></td></tr>";
                $output .= "<tr><td>Centre Contact:</td><td><input type='text' name='CGCentreCont' onblur='updateQualAttribute(this, {$this->id});return false;' value='".$this->get_attribute('CGCentreCont')."' /></td><td><span id='cgcentrecont' class='saveSetting'>Saved</span></td></tr>";
            $output .= "</table>";
