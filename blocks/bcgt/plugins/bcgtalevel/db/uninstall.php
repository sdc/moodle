 ...
 */
function xmldb_bcgtalevel_uninstall()
{
    global $DB;
    ...
    $quals = $DB->get_records_sql("SELECT qual.id FROM {block_bcgt_qualification} AS qual
        ...
        WHERE type.bcgttypefamilyid = ?", array(3));
    if($quals)
    {
    ...
    $types = $DB->get_records_sql("SELECT id FROM {block_bcgt_type} 
        WHERE bcgttypefamilyid = ?", array(3));
    if($types)
    {
    ...
    $DB->delete_records('block_bcgt_type_family', array("id"=>3));
    
    $DB->delete_records('block_bcgt_subtype', array("id"=>12));
    $DB->delete_records('block_bcgt_subtype', array("id"=>13));
    
    $DB->delete_records('block_bcgt_tabs', array('tabname'=>'ALEVEL'));
