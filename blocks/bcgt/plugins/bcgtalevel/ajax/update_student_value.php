...
$userValue = optional_param('uservalue', '', PARAM_TEXT);
...
if($grid == 'student')
{
    ...
    {
        $criteria = null;
        $formalAssessment = $qualification->get_assessment_by_criteria($criteriaID);
        if($formalAssessment)
        {
            $criteria = $formalAssessment->criteria;
            $criteria->set_user($user);
            $criteria->set_date();
            if($vType == 'targetgrade')
            {
                //update the targetgrade
                $criteria->set_targetgrade_id($valueID);
            }
            elseif($vType == 'userdefined')
            {
                //update he userdefined
                $criteria->set_user_defined_value($userValue);
            }
            elseif($vType == 'value')
            {
                //update the value
                $criteria->update_students_value($valueID);
            }
