 ...
 */
function xmldb_bcgtbtec_uninstall()
{
    global $DB;
    ...
    $quals = $DB->get_records_sql("SELECT qual.id FROM {block_bcgt_qualification} AS qual
        ...
        WHERE type.bcgttypefamilyid = ?", array(2));
    if($quals)
    {
    ...
    $types = $DB->get_records_sql("SELECT id FROM {block_bcgt_type} 
        WHERE bcgttypefamilyid = ?", array(2));
    if($types)
    {
    ...
    $DB->delete_records('block_bcgt_type_family', array("id"=>2));
    
    $DB->delete_records('block_bcgt_subtype', array("id"=>2));
    $DB->delete_records('block_bcgt_subtype', array("id"=>3));
    $DB->delete_records('block_bcgt_subtype', array("id"=>4));
    $DB->delete_records('block_bcgt_subtype', array("id"=>5));
    $DB->delete_records('block_bcgt_subtype', array("id"=>6));
    $DB->delete_records('block_bcgt_subtype', array("id"=>7));
    $DB->delete_records('block_bcgt_subtype', array("id"=>8));
    $DB->delete_records('block_bcgt_subtype', array("id"=>9));
    $DB->delete_records('block_bcgt_subtype', array("id"=>10));
    $DB->delete_records('block_bcgt_subtype', array("id"=>11));
