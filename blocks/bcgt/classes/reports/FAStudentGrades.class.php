...
/**
 * Description of FAStudentGrades
 *
 * @author mchaney
 */
class FAStudentGrades extends CoreReports{
    //put your code here
    protected $freezepane = 'D2';
    
    function FAStudentGrades()
    {
    ...
    function get_name()
    {
        return "Formal Assessment Student Grades";
    }
    
    function get_description()
    {
        return "All students, with their subjects and their current Grades";
    }
            ...
            foreach($students AS $student)
            {
                ...
                foreach($qualifications AS $qual)
                {
                    ...
                    if(bcgt_is_user_on_qual($qual->id, $student->id))
                    {
                        ...
                        $value = Qualification::get_current_fa_grade($qual->id, $student->id);
                        if($value)
                        {
//                            $value = end($values);
                            $data[] = $value->shortvalue;
                            $totalGrades[$value->ranking] = $value->shortvalue;;
                            $param = $value->shortvalue;;
            ...
            foreach($students AS $student)
            {
                ...
                if($quals)
                {
                    ...
                    foreach($quals AS $qual)
                    {
                        ...
                        {
//                            $value = end($values);
                            $data[] = $cellContent.$value->shortvalue;
                        }
                        else 
                        {
        ...
        if($displayType == 'bysubject')
        {     
            //now append new rows for the summary:
            //
//            $studentData[] = array(); //blank row
            ...
            //now append on the end of all rows. 
            $header[] = '';
            foreach($totalGrades AS $ranking=>$grade)
            {
                $header[] = $grade;
