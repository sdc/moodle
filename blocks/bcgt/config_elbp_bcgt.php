...
try {
    $OBJ = \ELBP\Plugins\Plugin::instaniate("elbp_bcgt");
...
$PAGE->set_url($CFG->wwwroot . $OBJ->getPath() . 'config_elbp_bcgt.php');
...
try {
    $TPL->load( $CFG->dirroot . $OBJ->getPath() . '/tpl/elbp_bcgt/config.html' );
    $TPL->display();
} catch (\ELBP\ELBPException $e){
    echo $e->getException();
}
