...
$event = \mod_activitytypeheader\event\course_module_instance_list_viewed::create($params);
$event->add_record_snapshot('course', $course);
$event->trigger();

//pull the language strings for the module
$strmodname      = get_string('modulename', 'mod_activitytypeheader');
$strmodnames     = get_string('modulenameplural', 'mod_activitytypeheader');
...
$PAGE->set_url('/mod/activitytypeheader/index.php', array('id' => $id));
...
if(!$newmodules = get_all_instances_in_course('activitytypeheader', $course)) {
	...
	notice(get_string('nonewmodules', 'activitytypeheader'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
$currentsection = '';
//iterate each of the activitytypeheader modules
foreach($modinfo->instances['activitytypeheader'] as $cm) {
