...
$event = \mod_activitytask\event\course_module_instance_list_viewed::create($params);
...
$strname         = get_string('activitytask', 'mod_activitytask');
$strnameplural   = get_string('activitytasks', 'mod_activitytask');
...
$PAGE->set_url('/mod/activitytask/index.php', array('id' => $id));
...
if(!$newmodules = get_all_instances_in_course('activitytask', $course)) {
	...
	notice(get_string('nonewmodules', 'activitytask'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
$currentsection = '';
//iterate each of the activitytask modules
foreach($modinfo->instances['activitytask'] as $cm) {
